...
import { calculatePriceDifference } from "../calculate_price_difference";
...
export const scanP2pOrders = async () => {
  ...
  if (!Tasks.getStatusUpdatePrice() && nextTask) {
    ...
    if (
      existingOrder &&
      beatBy <
        calculatePriceDifference(existingOrder.adv.price, orders[1].adv.price)
    ) {
      Tasks.setStatusUpdatePrice(true);

      pages[pair.asset] = 1;

      const orderPrice = convertStringToNumber(orders[1].adv.price);
      const tradeType = orders[1].adv.tradeType;
      const newPrice =
