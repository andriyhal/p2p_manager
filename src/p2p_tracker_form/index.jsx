...
const TrashIcon = () => <svg fill="#FF0000" version="1.1" id="Capa_1" width="28px" height="28px" viewBox="0 0 470.713 470.714" >
<g>
	<g>
...
const EditIcon = () => <svg width="24px" height="24px" viewBox="0 0 20 20" fill="none">
...
const PlusIcon = ({color = "#008000"}) => <svg width="28px" height="28px" viewBox="0 0 24 24" fill="none">
...
const taskFilter = data => {
    const existingIndex = tasksInfo.readData().findIndex(task => task.orderId === data.orderId);

    if (existingIndex === -1) {
        tasksInfo.saveData([...tasksInfo.readData(), data]);
    } else {
        tasksInfo.saveData(tasksInfo.readData().map(item => 
            item.orderId === data.orderId ? {...item, priceThreshold: data.priceThreshold, targetOrderAmount: data.targetOrderAmount } : item
        ));
    }
...
export const P2PTrackerForm = (props) => {
    ...
    const [isTask, setIsTask] = useState(!!tasksInfo.readData().filter(task => task.orderId === props.orderId)?.[0])

    const handleSaveTaskToLocalStorage = submitData => {
        console.log({submitData});
        const stringWithoutCommasPriceThreshold = submitData.priceThreshold.replace(/,/g, '');
        const parsedNumberPriceThreshold = parseFloat(stringWithoutCommasPriceThreshold);

        const stringWithoutCommasTargetOrderAmount = submitData.targetOrderAmount.replace(/,/g, '');
        const parsedNumberTargetOrderAmount = parseFloat(stringWithoutCommasTargetOrderAmount);       

        taskFilter({priceThreshold: parsedNumberPriceThreshold, targetOrderAmount: parsedNumberTargetOrderAmount, ...props}); 
    }
    ...
    const deleteTask = () => {
        ...
        tasksInfo.saveData(
            tasksInfo.readData()
            .filter(task => task.orderId !== props.orderId)
        );
    }

    return (
        <P2PTrackerFormContainer>
            <FormContainer onSubmit={handleSubmit(handleSaveTaskToLocalStorage)} id={props.orderId}>
                <FormInputText label="price threshold" name="priceThreshold" control={control}/>
                <FormInputText label="target order amount" name="targetOrderAmount" control={control}/>
                { 
                    isTask ? <>
