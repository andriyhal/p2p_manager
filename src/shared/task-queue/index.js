...
const existingTasks = JSON.parse(localStorage.getItem(TASKS_INFO_STORAGE_KEY)) || [];
...
export const processNextTask = () => {
	const queuedTasks = JSON.parse(localStorage.getItem(TASKS_INFO_STORAGE_KEY)) || [];

	if (queuedTasks.length > 0) {
		const nextTask = queuedTasks.shift();
		queuedTasks.push(nextTask);

		localStorage.setItem(TASKS_INFO_STORAGE_KEY, JSON.stringify(queuedTasks));
